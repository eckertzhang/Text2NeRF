...
class Pix2Pix4DepthModel(BaseModel):
    ...
    def set_input(self, depth, image):
        depth = torch.from_numpy(depth).unsqueeze(0).unsqueeze(0)
        image = torch.from_numpy(image).unsqueeze(0)

        # inner = (depth - torch.min(depth))/(torch.max(depth)-torch.min(depth))
        # outer = (outer - torch.min(outer))/(torch.max(outer)-torch.min(outer))

        # inner = self.normalize(inner)
        # outer = self.normalize(outer)

        self.real_A = torch.cat((image, depth), 1).to(self.device)

    def set_input_tensor(self, depth_guide, img):
        """
        depth_guide: [N, 1, H, W]
        img: [N, 3, H, W]
        """

        # depth_guide = (depth_guide - torch.min(depth_guide))/(torch.max(depth_guide)-torch.min(depth_guide))

        # depth_guide = self.normalize(depth_guide)
        # img = self.normalize(img)

        self.real_A = torch.cat((img, depth_guide), 1).to(self.device)
