...
def run(dataset, option):
    ...
    for image_ind, images in enumerate(dataset):
        ...
        # Output the result
        path = os.path.join(result_dir, imageandpatchs.name)#.replace('rgb','depth'))
        if option.output_resolution == 1:
            midas.utils.write_depth(path,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='test_img', help='input files directory '
                                                                    'Images can be .png .jpg .tiff')
    parser.add_argument('--output_dir', type=str, default='outputs/test_img', help='result dir. result depth will be png.'
    ...
    dataset_ = ImageDataset_single(option_.data_dir, 'test')
