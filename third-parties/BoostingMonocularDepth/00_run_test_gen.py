"""
Author: Eckert ZHANG
Date: 2022-08-07 09:04:06
LastEditTime: 2022-08-07 09:53:13
LastEditors: Eckert ZHANG
FilePath: /BoostingMonocularDepth/00_run_test_gen.py
Description: 
"""
"""
Author: Eckert ZHANG
Date: 2022-06-28 10:30:17
LastEditTime: 2022-07-12 16:51:53
LastEditors: Eckert ZHANG
FilePath: /BoostingMonocularDepth/00_run_test.py
Description: 
"""
...
# OUR
from utils import ImageandPatchs, ImageDataset_single, generatemask, getGF_fromintegral, calculateprocessingres, rgb2gray,\
    applyGridpatch, ImageDataset
...
def run(dataset, option):
    ...
    opt.checkpoints_dir = option.checkpoints_dir
    global pix2pixmodel
    pix2pixmodel = Pix2Pix4DepthModel(opt)
    pix2pixmodel.save_dir = opt.checkpoints_dir+'/mergemodel'
    pix2pixmodel.load_networks('latest')
    pix2pixmodel.eval()

    # Decide which depth estimation network to load
    if option.depthNet == 0:
        midas_model_path = "weights/weights_BoostingMonocularDepth/midas/model.pt"
        global midasmodel
        ...
        global leresmodel
        leres_model_path = opt.checkpoints_dir+"/res101.pth"
        checkpoint = torch.load(leres_model_path)
        leresmodel = RelDepthModel(backbone='resnext101')
        leresmodel.load_state_dict(strip_prefix_if_present(
    ...
    for image_ind, images in enumerate(dataset):
        ...
        if option.output_resolution == 1:
            ...
            print('Dynamicly change merged-in resolution; scale:',
                  mergein_scale)
        else:
            mergein_scale = 1

        imageandpatchs = ImageandPatchs(option.data_dir, images.name, patchset,
                                        img, mergein_scale)
        ...
        for patch_ind in range(len(imageandpatchs)):
            ...
            if option.savepatchs:
                path = os.path.join(
                    ...
                    imageandpatchs.name + '_{:04}'.format(patch_id))
                midas.utils.write_depth(path,
                                        patch_estimation,
                                        bits=2,
                                        colored=option.colorize_results)
        ...
        path = os.path.join(result_dir,
                            imageandpatchs.name.replace('jpg', 'png'))
        if option.output_resolution == 1:
            midas.utils.write_depth(
                path,
...
def estimatesrl(img, msize):
    ...
    img_resized = cv2.resize(img, (msize, msize),
                             interpolation=cv2.INTER_CUBIC).astype('float32')
    tensor_img = img_transform(img_resized)

    # Forward pass
    input_img = torch.autograd.Variable(tensor_img.cuda().unsqueeze(0),
                                        volatile=True)
    with torch.no_grad():
    ...
    depth_norm = cv2.resize(depth_norm, (img.shape[1], img.shape[0]),
                            interpolation=cv2.INTER_CUBIC)
...
## conda activate bmd
## python 00_run_test_gen.py --data_dir 'inputs/scene_gen' --output_dir 'inputs/scene_gen'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pix2pixsize',
                        type=int,
                        default=1024,
    ...
    parser.add_argument(
        ...
        required=False,
        help=
        'use to select different base depth networks 0:midas 1:strurturedRL 2:LeRes'
    )
    ...
    parser.add_argument('--checkpoints_dir', type=str, default='./pix2pix/checkpoints')
